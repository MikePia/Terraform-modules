...
module "ec2" {
  source = "./ec2"
  #   my_public_key  = "~/.ssh/demo/test-key.pub"
  my_public_key  = "C:/python/keys/demo/test-key.pub"
...
module "sns_topic" {
  source       = "./sns"
  alarms_email = "c:/python/keys/demo/secretemail.txt"
}
...
module "iam" {
  source   = "./iam"
  username = ["codewizard", "devguru", "genius"]
}


module "s3" {
  source         = "./s3"
  s3_bucket_name = "a-bucket-for-all-tuts"
}
